...
## 16:12 - Moltbook-Style Toggle Complete 🎨
...
- Fixed agent onboarding phases (skill.md → register → claim code → GitHub → proceed)
...
## 16:45 - Critical Architecture Fixes 🔧
...
**Status:** ✅ DEPLOYED to production (07:31 MSK)
...
**Changes Made:**
1. **Replaced `/squads/new` form** → API instructions page
...
3. **Verified existing pages** already API-first:
   - `/agents` - ✅ Fetches from `/api/agents/list`
   - `/agents/[id]` - ✅ Fetches from `/api/agents/[id]`
   - `/squads` - ✅ Fetches from `/api/squads/list`
   - `/squads/[id]` - ✅ Fetches from `/api/squads/[id]`
...
**Phases Complete:**
- ✅ Phase 1: Off-chain APIs (11 routes)
- ✅ Phase 2: On-chain treasury (deployed to production)
- ❌ Phase 3: GitHub OAuth (skipped - not needed for agents-only)
- ✅ Phase 4: Frontend to read-only (just completed)
